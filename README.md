...
- `pbs.py`  
  ...
  Handles:
  - constructing the grid world (funnel-like map),
  - spawning multiple agents with starts/goals,
  - running PBS to produce joint trajectories,
  - running the chosen low-level solver (DQN or RRT),
  - creating GIFs/PNGs of rollouts,
  - writing log files to `run_logs/`.

- `rrt_baseline.py`  
  Self-contained **single-agent RRT** demo and visualization on a grid.  
  Used as a simple baseline and for debugging the RRT implementation.
...
- `dqn_run_run1.gif`, `rrt_run_run1.gif`, `solution.gif`  
  Example visualizations of:
  - a PBS + DQN rollout (`dqn_run_run1.gif`),  
  - a PBS + RRT rollout (`rrt_run_run1.gif`),  
  - the multi-agent PBS solution.

- `pbs_metrics.png`, `log_metrics.png`  
  Example metric plots generated by `analyze_logs.py`.
